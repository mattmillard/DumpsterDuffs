...
export default function AdminBookingsPage() {
	...
	const buildFullAddress = (booking: BookingRow) => {
		const addressParts = [
			...
			[booking.delivery_city, booking.delivery_state, booking.delivery_zip].filter(Boolean).join(" "),
		]
	...
	const loadBookings = async () => {
		...
		try {
			...
			setBookings([]);
			setLoadError("Could not load bookings right now. Please refresh.");
		} finally {
			setLoading(false);
		}
	};
	...
	const rows = filtered.map((booking) => ({
		...
		customer: (
			<div className="space-y-1">
				...
				{booking.customer_email && <p className="text-xs text-[#B3D4FF]">{booking.customer_email}</p>}
				{booking.customer_company && <p className="text-xs text-[#999999]">{booking.customer_company}</p>}
			</div>
		),
		address: (
			<div className="space-y-1">
			...
			{editingBooking && (
				<>
					...
					<div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#1A1A1A] border border-[#404040] rounded-lg p-8 max-w-md w-full z-50 shadow-2xl">
						<h2 className="text-2xl font-bold text-white mb-2">Update Booking Status</h2>
						...
						<div className="space-y-4 mb-6">
							<label className="block">
								<span className="text-sm font-medium text-[#999999] mb-2 block">Select New Status</span>
								<select
