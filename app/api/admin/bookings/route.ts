...
import { checkBookingCapacity, isActiveBookingStatus } from "@/lib/admin/bookingOperations";
...
export async function GET() {
	try {
		...
		const minimalNormalized = (minimalQuery.data || []).map((booking) => ({
			...booking,
			customer_email: null,
...
export async function PATCH(request: Request) {
	try {
		...
		const movingIntoActiveState = isActiveBookingStatus(status) && !isActiveBookingStatus(existingBooking.status);
