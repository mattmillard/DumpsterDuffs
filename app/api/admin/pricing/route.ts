...
export async function POST(request: Request) {
  try {
    ...
    if (!name || !Number.isFinite(sizeYards) || sizeYards <= 0) {
      return NextResponse.json(
        { error: "Invalid size name or yard size" },
        { status: 400 },
      );
    }
    ...
    if (!Number.isFinite(perDayPrice) || perDayPrice < 0) {
      return NextResponse.json(
        { error: "Invalid extra day price" },
        { status: 400 },
      );
    }

    const slugId = `size-${sizeYards}`;
    const id = pricing.some((item) => item.id === slugId)
    ...
    const updated = [...pricing, created].sort(
      (a, b) => Number(a.size_yards) - Number(b.size_yards),
    );
    await setPricingConfig(updated);
    ...
    return NextResponse.json(
      { error: "Failed to create pricing size" },
      { status: 500 },
    );
  }
}
