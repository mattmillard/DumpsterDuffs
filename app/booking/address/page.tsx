...
import { BookingProgressBar, BookingContainer } from "@/components/booking/BookingProgressBar";
...
export default function BookingAddressPage() {
	...
	const parsePlaceComponents = (addressComponents: any[] = []) => {
		const byType = (type: string) => addressComponents.find((component) => component.types?.includes(type));
	...
	const initializeAutocomplete = () => {
		...
		const input = document.getElementById("booking-street-address") as HTMLInputElement | null;
	...
	return (
		<BookingContainer title="Delivery Address" description="Where should we deliver your dumpster?" showHeader={true}>
			<BookingProgressBar currentStep={2} totalSteps={STEPS.length} steps={STEPS} />
