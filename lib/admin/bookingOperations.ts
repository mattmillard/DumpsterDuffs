...
const ACTIVE_BOOKING_STATUSES = new Set(["pending", "scheduled", "in_progress"]);
...
type BookingCapacityResult = {
	bookable: boolean;
	reason?: "invalid_dates" | "size_not_available" | "date_blocked" | "no_capacity";
	message: string;
	conflictingDate?: string;
	capacity?: number;
	activeBookings?: number;
...
function isMissingTableError(error: unknown): boolean {
	...
	return maybeError.code === "42P01" || maybeError.message?.toLowerCase().includes("does not exist") === true;
}
...
async function fetchBlacklistEntries() {
	const { data, error } = await supabaseAdmin
		...
		.select("id, entry_type, value, normalized_value, reason, is_active, created_at")
...
export async function getCalendarSnapshot(month: string): Promise<CalendarSnapshot> {
	...
	const [pricing, inventory] = await Promise.all([getPricingConfig(), getInventoryConfig()]);
	...
	for (const size of activeSizes) {
		const matchedInventory =
			...
			inventory.find((item) => item.name.toLowerCase() === size.name.toLowerCase());

		capacityByYards.set(Number(size.size_yards), Math.max(0, Number(matchedInventory?.total_units ?? 0)));
	}
...
export async function blockDate(params: { date: string; size_yards?: number | null; reason?: string }) {
...
export async function freeDate(params: { id?: string; date?: string; size_yards?: number | null }) {
	if (params.id) {
...
export async function cancelBooking(bookingId: string, reason?: string) {
	...
	let result = await supabaseAdmin.from("bookings").update(updatePayload).eq("id", bookingId).select("id").single();
...
export async function addBlacklistEntry(params: {
	...
	const normalized_value = normalizeBlacklistValue(params.entry_type, params.value);
...
export async function toggleBlacklistEntry(id: string, isActive: boolean) {
	const { error } = await supabaseAdmin.from("booking_blacklist").update({ is_active: isActive }).eq("id", id);
...
export async function checkBookingCapacity(params: BookingCapacityParams): Promise<BookingCapacityResult> {
	const datePattern = /^\d{4}-\d{2}-\d{2}$/;
	if (
	...
	const sizeConfig = pricing.find((size) => size.is_active && Number(size.size_yards) === sizeYards);
	...
	const matchedInventory =
		...
		inventory.find((item) => item.name.toLowerCase() === sizeConfig.name.toLowerCase());
	...
	for (const date of dates) {
		const isDeliveryOrPickupDate = date === params.delivery_date || date === params.return_date;
...
export async function evaluateBlacklist(params: { phone?: string; email?: string; name?: string; address?: string }) {
